...
namespace ZstdNet
{
	internal static class ExternMethods
	{
		...
		private static void SetWinDllDirectory()
		{
			...
			{
                System.Diagnostics.Trace.TraceWarning($"{nameof(ZstdNet)}: Failed to get executing assembly location");
				return;
			}
            ...
            if(!SetDllDirectory(Path.Combine(path, platform)))
                System.Diagnostics.Trace.TraceWarning($"{nameof(ZstdNet)}: Failed to set DLL directory to '{path}'");
		}
