    ...
    internal class Common
    {
		public static void ApplyTextureConversions(SKBitmap bitmap, TextureCodec decodeFlags)
		{
            ...
            for (var i = 0; i < pixels.Length; i++)
            {
				...
				// For normal maps (when HemiOct or NormalizeNormals is applied), ensure alpha is opaque
				if (decodeHemiOct || reconstructZ)
				{
					pixels[i].a = 255; // Make normal maps fully opaque
				}
            }
        }
